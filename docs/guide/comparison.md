...
| Peer discovery                | Static `nodes` list, dialled at startup and redialled on `peeringSchedule`          | Public IPFS bootstrap peers and local network discovery   | As Kubo, plus a cluster peer set               | Whatever the author configures                       | Provider-operated                           | None; clients address a service endpoint       |
