...
| `repairFresh`           | No complete, healthy repair cycle inside `health.repairMaxAgeMs`, or a backlog exceeding `health.repairBacklogGraceCycles` | `GET /api/node/health` `replication.backlog` and `replication.ageMs`; raise `health.repairMaxAgeMs` above the largest expected cycle duration plus the schedule interval |
