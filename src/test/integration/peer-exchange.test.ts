...
import { createIpfsNode, createUnixFs, type IpfsNode } from '../../ipfs-node.js'
...
describe('two isolated nodes', () => {
  test('start with distinct peer ids and no bootstrap peers', () => {
...
describe('file exchange between known peers', () => {
  test('a file uploaded to node A is retrievable from node B', async () => {
  ...
  test('a transferred file keeps its pre-migration CID', async () => {
    const ifsA = createUnixFs(nodeA.node)
    const ifsB = createUnixFs(nodeB.node)
...
describe('peer identity', () => {
  test('is stable across a restart with the same store', async () => {
